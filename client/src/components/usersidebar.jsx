...
const Sidebar = ({ isOpen, setIsSidebarOpen }) => {
  ...
  return (
    <div
      className={`fixed z-40 top-0 left-0 w-52 h-screen bg-primary text-bg transition-transform transform ${
        isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
      }`}
    >
      <div className="p-5 text-center md:pt-8 mb-2">
        <span className="text-4xl font-extrabold border-b-2 pb-4 text-bg hidden md:block">
          मार्गदर्शक
        </span>
      </div>
        ...
        <div>
          ...
          {sidebarItems.map((item) => (
            <button
              ...
              className={`flex items-center gap-4 w-full text-left lg:mb-2 text-[16px] font-bold px-4 py-2 ${
